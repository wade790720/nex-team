...
**解答思路**:
1. **邊數檢查**: 陣列長度（邊的數量）必須等於頂點數（單一環的必要條件）
   - 邊數 = `A.length`（或 `B.length`，兩者相等）
   - 頂點數 = 所有出現在 A 和 B 中的不重複節點數量
   - 在單一環中，每個節點各有一條出邊和一條入邊，所以邊數 = 頂點數
2. **度數檢查**: 每個頂點的出度和入度都必須為 1
   - **出度（Out-degree）**: 從某個節點「出發」的邊的數量（在陣列 A 中出現的次數）
   - **入度（In-degree）**: 進入某個節點的邊的數量（在陣列 B 中出現的次數）
   - 在單一環中，每個節點都恰好有一條出邊和一條入邊
3. **連通性檢查**: 從任意節點開始追蹤，必須能走訪所有節點並回到起點
