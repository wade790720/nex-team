...
**解答思路**:
1. **建立鄰接表並檢查出度**: 一次遍歷建立 `nextNode` Map，同時檢查每個節點的出度是否為 1
   - 如果某個節點已經有出邊，則出度 > 1，不是單一環
   - 邊數 = `A.length`，頂點數 = `nextNode.size`
2. **邊數檢查**: 邊數必須等於頂點數（單一環的必要條件）
3. **連通性檢查**: 從任意節點開始追蹤，檢查是否能走訪所有節點並回到起點
   - 如果每個節點出度=1、邊數=頂點數、且能走訪所有節點回到起點，則入度也必須是1（數學性質）
