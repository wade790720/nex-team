...
**題目說明**:
判斷一個有向圖是否為單一環（cycle）；單一環的定義是：可以從某一點出發，遍歷所有其他節點一次，最後回到起點。
...
2. **一次遍歷建立資料結構**: 在一次遍歷中同時完成：
   - 收集所有節點到 `vertices` Set
   - 建立 `nextNode` Map（鄰接表），同時檢查出度是否為 1
     - 如果某個節點已經有出邊，則出度 > 1，不是單一環
   - 計算每個節點的入度到 `inDegree` Map
3. **邊數檢查**: 邊數必須等於節點數（單一環的必要條件）
   - 邊數 = `A.length`（或 `B.length`）
   - 節點數 = `vertices.size`
4. **連接數檢查**: 每個節點的出邊數量和入邊數量都必須為 1
   - 檢查 `inDegree.get(v) === 1` 確保入邊數量為 1
   - 檢查 `nextNode.has(v)` 確保出邊數量為 1（每個節點都有出邊）
5. **連通性檢查**: 從任意節點開始追蹤，檢查是否能遍歷所有節點並回到起點
   - 沿著 `nextNode` 遍歷 `vCount` 步
   - 必須不重複存取任何節點
   - 遍歷完所有節點後必須回到起點
